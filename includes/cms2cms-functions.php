 ...
 */
class CmsPluginFunctionsKentico
{
    ...
    private $jsConfig = '{
        ...
        "wp_feedback"  : "//wordpress.org/support/plugin/cms2cms-kentico-to-wp-migration/reviews",
    ...
    {
        $cms2cms_kentico_access_login = get_option('cms2cms-kentico-login');
        $cms2cms_kentico_access_key   = get_option('cms2cms-kentico-key');

        return array(
            'email'     => $cms2cms_kentico_access_login,
            'accessKey' => $cms2cms_kentico_access_key
        );
    }
    ...
    public function isActivated()
    {
        ...
        return ($cms2cms_kentico_access_key != false);
    }
    ...
    public function getOptions()
    {
        ...
        $login = get_option('cms2cms-kentico-login');
    ...
    public function saveOptions()
    {
        ...
        if ( $key && $login ) {
            ...
            add_option('cms2cms-kentico-key', $key);

            delete_option('cms2cms-kentico-login');
            add_option('cms2cms-kentico-login', $login);

            delete_option('cms2cms-kentico-depth');
            add_option('cms2cms-kentico-depth', $bridge_depth);
    ...
    {
        return $this->__('Kentico', 'cms2cms-kentico-migration');
    }
    ...
    {
        return $this->__('WordPress', 'cms2cms-kentico-migration');
    }
    ...
    {
        return sprintf(
            $this->__('CMS2CMS: Automated %s to %s Migration ', 'cms2cms-kentico-migration'),
    ...
    {
        return sprintf(
            $this->__('%s to %s', 'cms2cms-kentico-migration'),
            $this->getPluginSourceName(),
            $this->getPluginTargetName()
        );
    }
    ...
    public function logOut()
    {
        if (isset($_REQUEST['_wpnonce'])) {
            ...
            if ($this->verifyFormTempKey($nonce, 'cms2cms_kentico_logout')
                && $_POST['cms2cms_kentico_logout'] == 1
            ) {
                $this->clearOptions();
            }
        }
